...
from trainer_discrete import PLModel
from module_discrete import get_data
...
def main():
    config = json.load(open("configs/protein.json", "r"))
    ...
    if train_encoder:
        ...
        tb_logger = pl.loggers.TensorBoardLogger(
            name="protein_encoder", save_dir="lightning_logs/"
        )
        trainer = pl.Trainer(
            max_epochs=50,
            gpus=[free_gpu_id],
        ...
        # )
        torch.save(model.model.cpu().state_dict(), "models/protein_trained_encoder.pt")

    model = PLModel(stage=2, **config)
    model.model.load_state_dict(torch.load("models/protein_trained_encoder.pt"))
    ...
    tb_logger = pl.loggers.TensorBoardLogger(
        name="protein_sesm", save_dir="lightning_logs/"
    )
    trainer = pl.Trainer(
        # max_epochs=500,
        # gpus=[0, 1, 2],
