...
class Embedder(nn.Module):
    def __init__(self, d_input, d_embed, d_kernel):
        super().__init__()
        self.embed = nn.Embedding(d_input, d_embed, padding_idx=00)
        self.proj = nn.Sequential(
            nn.Conv1d(d_embed, d_embed, d_kernel),
...
class Model(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.embed = Embedder(d_input, d_embed, d_kernel)
    ...
    def forward(self, sentence, sent_mask=None, heads=False):
        ...
        ## 约束
        L_diversity = self._diversity_term(sequential_selector) + self._diversity_term(
            x_sequential_selected_encoded, d="cosine"
        )
